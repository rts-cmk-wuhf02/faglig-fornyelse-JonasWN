...
const fn = (order, down, originalIndex, curIndex, y) => index =>
  down && index === originalIndex
    ? { y: curIndex * 75 + y, scale: 1.1, zIndex: '1', shadow: 15, immediate: n => n === 'y' || n === 'zIndex' }
    : { y: order.indexOf(index) * 75, scale: 1, zIndex: '0', shadow: 1, immediate: false }
...
const Tasks = ({ task }) => {
  ...
  const bind = useGesture(({ args: [originalIndex], down, delta: [, y] }) => {
    ...
    const curRow = clamp(Math.round((curIndex * 100 + y) / 100), 0, order.current.length - 1)
